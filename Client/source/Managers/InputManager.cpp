...
void InputManager::setButtonClick(sf::Mouse::Button button, sf::Vector2i position) {
    auto& buttons = sInstance->m_mouseButtons;
    auto pairFound = buttons.find(button);
    // If button exists in the map
    if (pairFound != buttons.end()) {
        if (!buttons[button].hold) {
            pairFound->second.clickPosition = position;
            pairFound->second.releasePosition = {0, 0};
            pairFound->second.hold = true;
        }
    }
}

void InputManager::setButtonRelease(sf::Mouse::Button button, sf::Vector2i position) {
    auto& buttons = sInstance->m_mouseButtons;
    auto pairFound = buttons.find(button);
    // If button exists in the map
    if (pairFound != buttons.end()) {
        pairFound->second.releasePosition = position;
        pairFound->second.hold = false;
    }
}

MouseButton& InputManager::getButton(sf::Mouse::Button button) {
    auto& buttons = sInstance->m_mouseButtons;
    auto pairFound = buttons.find(button);
    // If button exists in the map
    if (pairFound != buttons.end()) {
...
void InputManager::reset() {
    for (auto& button : sInstance->m_mouseButtons) {
        button.second.clickPosition = {0, 0};
        button.second.releasePosition = {0, 0};
        button.second.hold = false;
    }
}

sf::Keyboard::Key InputManager::getKeyAction(const std::string& key) {
    return sInstance->m_actionMap[key];
}

void InputManager::initMap() {
